...
class QQAdapterPatchAdapter(BasePlatformAdapter):
    """QQ Bot adapter backed by the official QQ Bot WebSocket Gateway + REST API.

    Fork of the built-in ``QQAdapter`` (v0.20.0) with patches on top:
    - group-member allowlist: when ``group_member_allow_from`` is non-empty,
      only the listed ``member_openid`` values may trigger the bot inside an
      allowlisted group;
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Timestamp of the last inbound message per chat (for the 5-minute
        # QQ passive-reply window when falling back to msg_id in send()).
        self._last_msg_id_ts: Dict[str, float] = {}
        # group_openid → most recent sender member_openid.  Legacy fallback
        # only: response-specific fallback should use the trigger msg_id/session
        # provenance so concurrent users in one group do not receive each
        # other's replies.
        self._group_last_sender: Dict[str, str] = {}
        # group msg_id → (group_openid, member_openid, timestamp).  Bounded TTL
        # cache used to resolve a response's reply_to anchor back to the exact
        # group member that triggered that response, avoiding a persistent
        # per-session map.
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if event.message_id and event.source.chat_id:
            ...
            self._last_msg_id_ts[event.source.chat_id] = time.time()
    ...
    async def _handle_group_message(
        ...
        if not self._is_group_allowed(group_openid, member_openid):
            # Log the rejection (and the group/member ids) so operators can
            # discover member_openid values and diagnose allowlist misses.
            logger.info(
                "[%s] Group message rejected: group=%s member=%s policy=%s "
                "group_allow=%s member_allow=%s content=%r",
                self._log_tag,
                group_openid,
                member_openid,
                self._group_policy,
                ",".join(self._group_allow_from) or "-",
                ",".join(self._group_member_allow_from) or "-",
                content[:80],
            )
            return
    ...
    async def send(
        ...
        """
        # Prefer response-specific provenance over the legacy per-group last
        # sender, but only for group chats.  Do not run group ledger/fallback
        # resolution for known C2C chats: delivery_obligations can contain DM
        # rows with the same chat_id/content, and treating those as ambiguous
        # group rows pollutes _chat_type_map[dm_openid] = "group", which routes
        ...
        # Passive-reply fallback: QQ rejects outbound group/C2C messages that
        # carry no msg_id ("主动消息失败, 无权限" for groups) unless the bot is
        # authorized for active messages.  When the caller gave no reply_to but
        # we have a recent inbound message for this chat (within QQ's 5-minute
        # passive-reply window), reuse its msg_id so the send counts as a
        # passive reply instead of an active message.
        if not reply_to:
            last_msg_id = self._last_msg_id.get(chat_id)
            last_ts = self._last_msg_id_ts.get(chat_id, 0.0)
            if last_msg_id and (time.time() - last_ts) <= 300.0:
                reply_to = last_msg_id
        if fallback_member_openid is None:
    ...
    async def _send_fallback_dm(
        ...
        Prefer the member tied to the current response's trigger/session.  The
        legacy group-last-sender map is only a final fallback for old call sites.
        """
        member_openid = member_openid or (
            self._group_last_sender.get(group_openid) if allow_legacy_fallback else None
        )
        if not member_openid:
            logger.warning(
                "[%s] Group send failed but no known sender for %s — cannot "
                "fall back to DM",
                self._log_tag, group_openid,
            )
            return None

        # Prefer a passive reply: reuse the sender's most recent C2C msg_id
        # when it is still within QQ's 5-minute window.
        c2c_msg_id = self._last_msg_id.get(member_openid)
        c2c_ts = self._last_msg_id_ts.get(member_openid, 0.0)
        dm_reply_to = (
            c2c_msg_id
    ...
    async def _send_chunk(
        ...
        Passive-reply quota exhaustion (QQ: ``被动回复时间或者次数超过限制``)
        is handled specially: retrying with the same msg_id only burns the
        remaining per-message reply quota (单聊 4 次/60 分钟, 群 5 次/5 分钟),
        so we drop msg_id and retry once as an active message instead.
        The result is flagged non-retryable so the caller (base retry loop)
        does not hammer the same msg_id again.
        """
        last_exc: Optional[Exception] = None
        chat_type = self._guess_chat_type(chat_id)
        passive_quota_exhausted = False
        ...
        for attempt in range(3):
            try:
                ...
                last_exc = exc
                err = str(exc).lower()
                # QQ passive-reply quota exhausted: same-msg_id retries are
                # futile (each attempt consumes the per-message quota) and the
                # error is not detected by the generic retryable keywords
                # below, which is what caused the old retry-storm.  Drop the
                # msg_id and try once as an active message.
                if "被动回复时间或者次数超过限制" in err:
                    passive_quota_exhausted = True
                    if reply_to:
                        logger.warning(
                            "[%s] Passive-reply quota exhausted for %s — "
                            "retrying once without msg_id (active message)",
                        ...
                        try:
                            ...
                            last_exc = active_exc
                            logger.error(
                                "[%s] Active-message fallback failed for %s: %s",
                                self._log_tag, chat_id, active_exc,
                            )
                    # Regardless of fallback outcome, never retry this msg_id.
                    break
        ...
        # Passive-quota exhaustion must not bubble up as retryable: the caller
        # (base retry loop) would re-send the same msg_id and re-trip the limit.
        retryable = (
            not any(
                k in error_msg.lower() for k in ("invalid", "forbidden", "not found")
            )
            and not passive_quota_exhausted
        )
    ...
    async def _send_media(
        ...
        chat_type = self._guess_chat_type(chat_id)
        if not reply_to and chat_type in {"c2c", "group"}:
            last_msg_id = self._last_msg_id.get(chat_id)
            last_ts = self._last_msg_id_ts.get(chat_id, 0.0)
            if last_msg_id and (time.time() - last_ts) <= 300.0:
                reply_to = last_msg_id
        ...
        try:
            ...
            try:
                send_data = await self._api_request("POST", send_path, body)
            except Exception as send_exc:
                if reply_to and "被动回复时间或者次数超过限制" in str(send_exc).lower():
                    logger.warning(
                        "[%s] Passive media-reply quota exhausted for %s — "
                        "retrying once without msg_id (active message)",
                        self._log_tag, chat_id,
                    )
    ...
    def _remember_group_msg_sender(
            self, msg_id: str, group_openid: str, member_openid: str,
    ) -> None:
        """Remember which group member produced an inbound message id.

        The gateway replies to a group message by passing that inbound msg_id as
        ``reply_to``.  Keeping a bounded msg_id → member map lets fallback-DM
        target the exact session owner instead of the group's last sender.
        """
        msg_id = str(msg_id or "").strip()
        group_openid = str(group_openid or "").strip()
        member_openid = str(member_openid or "").strip()
        if not msg_id or not group_openid or not member_openid:
            return
        now = time.time()
    ...
    def _prune_group_msg_sender(self, now: Optional[float] = None) -> None:
        ...
        cutoff = now - self._group_msg_sender_ttl
        for key, (_, _, ts) in list(self._group_msg_sender.items()):
            if ts < cutoff:
                self._group_msg_sender.pop(key, None)
        overflow = len(self._group_msg_sender) - self._group_msg_sender_max
        if overflow > 0:
            oldest = sorted(
                self._group_msg_sender.items(), key=lambda item: item[1][2]
            )[:overflow]
            for key, _ in oldest:
                self._group_msg_sender.pop(key, None)

    def _member_for_reply_anchor(
            self, group_openid: str, reply_to: Optional[str],
    ) -> Optional[str]:
        reply_to = str(reply_to or "").strip()
        if not reply_to:
            return None
        item = self._group_msg_sender.get(reply_to)
        if not item:
            return None
        cached_group, member_openid, ts = item
        if cached_group != group_openid:
            return None
        if (time.time() - ts) > self._group_msg_sender_ttl:
            self._group_msg_sender.pop(reply_to, None)
            return None
        return member_openid

    @staticmethod
    def _session_member_from_key(session_key: str, group_openid: str) -> Optional[str]:
        parts = str(session_key or "").split(":")
        if len(parts) >= 6 and parts[:4] == ["agent", "main", "qqbot", "group"]:
            if parts[4] == group_openid:
                member = parts[-1].strip()
                return member or None
        return None

    @staticmethod
    def _ledger_content_candidates(content: str) -> List[str]:
        raw = str(content or "")
        candidates = [raw]
        if raw.startswith("♻️ Recovered reply"):
            _, sep, tail = raw.partition("\n\n")
            if sep and tail:
                candidates.append(tail)
        return list(dict.fromkeys(candidates))

    def _member_from_delivery_ledger(
            self, group_openid: str, content: str,
    ) -> Tuple[Optional[str], bool]:
        """Resolve current group response owner from the durable ledger.

        Returns ``(member, authoritative)``.  ``authoritative=False`` means the
        lookup proved this is a group chat, but could not uniquely identify the
        member (for example, multiple same-content obligations in the same
        group).  In that case callers must not fall back to group-last-sender.
        """
        try:
            from hermes_constants import get_hermes_home
            ...
            if not db_path.exists():
                return None, True
            candidates = self._ledger_content_candidates(content)
            placeholders = ",".join("?" for _ in candidates)
            query = (
                "SELECT session_key FROM delivery_obligations "
                "WHERE platform=? AND chat_id=? AND content IN (" + placeholders + ") "
            ...
            with sqlite3.connect(uri, uri=True, timeout=0.2) as conn:
                rows = conn.execute(
                    query, ["qqbot", group_openid, *candidates]
                ).fetchall()
            group_rows = []
            dm_rows = []
            other_rows = []
            ...
            for row in rows:
                ...
                if len(parts) >= 5 and parts[:4] == ["agent", "main", "qqbot", "dm"]:
                    dm_rows.append(session_key)
                    continue
                member = self._session_member_from_key(session_key, group_openid)
                if member:
                    group_rows.append(session_key)
                    members.add(member)
                elif len(parts) >= 5 and parts[:4] == ["agent", "main", "qqbot", "group"]:
                    group_rows.append(session_key)
                else:
                    other_rows.append(session_key)
            if len(members) == 1:
                return next(iter(members)), True
            if group_rows:
                logger.warning(
                    "[%s] Ambiguous QQ group ledger owner for %s (%d members); "
                    "will not DM-fallback to group-last-sender",
                    self._log_tag, group_openid, len(members),
                )
                self._chat_type_map[group_openid] = "group"
                return None, False
            if dm_rows:
                self._chat_type_map[group_openid] = "c2c"
                return None, False
            if other_rows:
                return None, False
            return None, True
        except Exception:
            logger.debug("[%s] Could not resolve QQ group sender from ledger", self._log_tag, exc_info=True)
            return None, True

    def _metadata_group_member(
            self, chat_id: str, metadata: Optional[Dict[str, Any]],
    ) -> Optional[str]:
        if not isinstance(metadata, dict):
            return None
        chat_type = str(
            metadata.get("source_chat_type") or metadata.get("chat_type") or ""
        ).strip().lower()
        source_chat_id = str(
            metadata.get("source_chat_id") or metadata.get("chat_id") or ""
        ).strip()
        member_openid = str(
            metadata.get("source_user_id") or metadata.get("user_id") or ""
        ).strip()
        if chat_type == "group" and (not source_chat_id or source_chat_id == str(chat_id)):
            self._chat_type_map[chat_id] = "group"
            return member_openid or None
        return None

    def _resolve_group_fallback_member(
            self,
            chat_id: str,
            content: str,
        ...
        member_openid = self._metadata_group_member(chat_id, metadata)
        if member_openid:
            return member_openid, True
        member_openid = self._member_for_reply_anchor(chat_id, reply_to)
        if member_openid:
            self._chat_type_map[chat_id] = "group"
            return member_openid, True
        member_openid, authoritative = self._member_from_delivery_ledger(chat_id, content)
        if member_openid:
            self._chat_type_map[chat_id] = "group"
            return member_openid, True
    ...
    def _is_group_allowed(self, group_id: str, user_id: str) -> bool:
        ...
        if self._group_policy == "allowlist":
            if not self._entry_matches(self._group_allow_from, group_id):
                return False
            # Group-member allowlist: when non-empty, only the listed
            # member_openid values may trigger the bot inside an
            # allowlisted group.  Empty list = no per-member restriction.
            return (not self._group_member_allow_from) or self._entry_matches(
                self._group_member_allow_from, user_id
            )
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        adapter_factory=lambda cfg: QQAdapterPatchAdapter(cfg),
